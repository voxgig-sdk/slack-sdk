// Command Slack-mcp is an MCP server exposing the Slack
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Slack_list  — list records of an entity
//	Slack_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: SLACK_APIKEY carries the API key and
	// SLACK_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("SLACK_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("SLACK_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewSlackSDK(opts)

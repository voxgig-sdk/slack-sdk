...
[MCP](https://modelcontextprotocol.io) server exposing the Slack SDK as
two agent tools — `slack_list` and `slack_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/slack-mcp)
make build
...
claude mcp add --scope user slack \
  -- /absolute/path/to/slack-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./slack-mcp -transport http -addr :8080
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/slack-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user slack \
     -- "$PWD"/dist/*/slack-mcp -transport stdio
   ```
...
| Tool | Args | Returns |
|------|------|---------|
| `slack_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `slack_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SLACK_APIKEY` | API key sent with every request. |
| `SLACK_BASE` | Optional override of the API base URL. |
...
```sh
./slack-mcp -transport http -addr :18080 &

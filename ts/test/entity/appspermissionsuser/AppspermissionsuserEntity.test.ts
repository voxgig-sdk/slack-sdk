...
describe('AppspermissionsuserEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'appspermissionsuser.' + op, live)) return
    }
    ...
    let appspermissionsuser_ref01_data = Object.values(setup.data.existing.appspermissionsuser)[0] as any

    // LOAD
    const appspermissionsuser_ref01_ent = client.Appspermissionsuser()
    const appspermissionsuser_ref01_match_dt0: any = {}

...
describe('AppseventauthorizationEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'appseventauthorization.' + op, live)) return
    }
    ...
    let appseventauthorization_ref01_data = Object.values(setup.data.existing.appseventauthorization)[0] as any

    // LOAD
    const appseventauthorization_ref01_ent = client.Appseventauthorization()
    const appseventauthorization_ref01_match_dt0: any = {}
    ...
    assert(null != appseventauthorization_ref01_data_dt0)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/appseventauthorization/AppseventauthorizationTestData.json')
  ...
  let idmap = transform(
    ['appseventauthorization01','appseventauthorization02','appseventauthorization03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',

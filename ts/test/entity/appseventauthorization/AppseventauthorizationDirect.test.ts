...
describe('AppseventauthorizationDirect', async () => {
  ...
  test('direct-load-appseventauthorization', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-appseventauthorization', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SLACK_TEST_APPSEVENTAUTHORIZATION_ENTID']

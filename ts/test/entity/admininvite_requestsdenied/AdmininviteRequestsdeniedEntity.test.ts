...
describe('AdmininviteRequestsdeniedEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'admininvite_requestsdenied.' + op, live)) return
    }
    ...
    let admininvite_requestsdenied_ref01_data = Object.values(setup.data.existing.admininvite_requestsdenied)[0] as any

    // LOAD
    const admininvite_requestsdenied_ref01_ent = client.AdmininviteRequestsdenied()
    const admininvite_requestsdenied_ref01_match_dt0: any = {}
    ...
    assert(null != admininvite_requestsdenied_ref01_data_dt0)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/admininvite_requestsdenied/AdmininviteRequestsdeniedTestData.json')
  ...
  let idmap = transform(
    ['admininvite_requestsdenied01','admininvite_requestsdenied02','admininvite_requestsdenied03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',

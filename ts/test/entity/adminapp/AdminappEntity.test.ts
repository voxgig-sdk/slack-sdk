...
describe('AdminappEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'adminapp.' + op, live)) return
    }
    ...
    assert(null != adminapp_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/adminapp/AdminappTestData.json')
  ...
  let idmap = transform(
    ['adminapp01','adminapp02','adminapp03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',

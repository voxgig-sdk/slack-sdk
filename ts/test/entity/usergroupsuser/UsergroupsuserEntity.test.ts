...
describe('UsergroupsuserEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'usergroupsuser.' + op, live)) return
    }
    ...
    // LIST
    const usergroupsuser_ref01_match: any = {}

    const usergroupsuser_ref01_list = (await usergroupsuser_ref01_ent.list(usergroupsuser_ref01_match)).map((e: any) => e.data())

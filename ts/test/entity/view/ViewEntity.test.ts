...
describe('ViewEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    let view_ref01_data = Object.values(setup.data.existing.view)[0] as any

    // LOAD
    const view_ref01_ent = client.View()
    const view_ref01_match_dt0: any = {}
    const view_ref01_data_dt0 = (await view_ref01_ent.load(view_ref01_match_dt0)).data()
    assert(null != view_ref01_data_dt0)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/view/ViewTestData.json')
  ...
  idmap = env['SLACK_TEST_VIEW_ENTID']

  const live = 'TRUE' === env.SLACK_TEST_LIVE

  const transport = createLiveTransport()
  if (live) {
    const rawIds = process.env['SLACK_TEST_VIEW_ENTID']

...
describe('AdminteamsadminEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'adminteamsadmin.' + op, live)) return
    }
    ...
    let adminteamsadmin_ref01_data = Object.values(setup.data.existing.adminteamsadmin)[0] as any

    // LOAD
    const adminteamsadmin_ref01_ent = client.Adminteamsadmin()
    const adminteamsadmin_ref01_match_dt0: any = {}
    const adminteamsadmin_ref01_data_dt0 = (await adminteamsadmin_ref01_ent.load(adminteamsadmin_ref01_match_dt0)).data()
    assert(null != adminteamsadmin_ref01_data_dt0)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/adminteamsadmin/AdminteamsadminTestData.json')
  ...
  let idmap = transform(
    ['adminteamsadmin01','adminteamsadmin02','adminteamsadmin03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',

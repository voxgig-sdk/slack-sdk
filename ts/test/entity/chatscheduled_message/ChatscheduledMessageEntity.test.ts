...
describe('ChatscheduledMessageEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'chatscheduled_message.' + op, live)) return
    }
    ...
    let chatscheduled_message_ref01_data = Object.values(setup.data.existing.chatscheduled_message)[0] as any

    // LIST
    const chatscheduled_message_ref01_ent = client.ChatscheduledMessage()
    const chatscheduled_message_ref01_match: any = {}

    const chatscheduled_message_ref01_list = (await chatscheduled_message_ref01_ent.list(chatscheduled_message_ref01_match)).map((e: any) => e.data())

...
describe('AdminappsapprovedEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'adminappsapproved.' + op, live)) return
    }
    ...
    let adminappsapproved_ref01_data = Object.values(setup.data.existing.adminappsapproved)[0] as any

    // LOAD
    const adminappsapproved_ref01_ent = client.Adminappsapproved()
    const adminappsapproved_ref01_match_dt0: any = {}

...
describe('AdminappsapprovedDirect', async () => {
  ...
  test('direct-load-adminappsapproved', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-adminappsapproved', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SLACK_TEST_ADMINAPPSAPPROVED_ENTID']

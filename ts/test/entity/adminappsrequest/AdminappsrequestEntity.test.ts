...
describe('AdminappsrequestEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'adminappsrequest.' + op, live)) return
    }
    ...
    let adminappsrequest_ref01_data = Object.values(setup.data.existing.adminappsrequest)[0] as any

    // LOAD
    const adminappsrequest_ref01_ent = client.Adminappsrequest()
    const adminappsrequest_ref01_match_dt0: any = {}

...
describe('DialogEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'dialog.' + op, live)) return
    }
    ...
    let dialog_ref01_data = Object.values(setup.data.existing.dialog)[0] as any

    // LOAD
    const dialog_ref01_ent = client.Dialog()
    const dialog_ref01_match_dt0: any = {}

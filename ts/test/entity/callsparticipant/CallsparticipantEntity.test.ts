...
describe('CallsparticipantEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'callsparticipant.' + op, live)) return
    }
    ...
    assert(null != callsparticipant_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/callsparticipant/CallsparticipantTestData.json')
  ...
  let idmap = transform(
    ['callsparticipant01','callsparticipant02','callsparticipant03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',

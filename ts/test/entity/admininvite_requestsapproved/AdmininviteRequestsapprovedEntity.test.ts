...
describe('AdmininviteRequestsapprovedEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'admininvite_requestsapproved.' + op, live)) return
    }
    ...
    let admininvite_requestsapproved_ref01_data = Object.values(setup.data.existing.admininvite_requestsapproved)[0] as any

    // LOAD
    const admininvite_requestsapproved_ref01_ent = client.AdmininviteRequestsapproved()
    const admininvite_requestsapproved_ref01_match_dt0: any = {}
    ...
    assert(null != admininvite_requestsapproved_ref01_data_dt0)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/admininvite_requestsapproved/AdmininviteRequestsapprovedTestData.json')
  ...
  let idmap = transform(
    ['admininvite_requestsapproved01','admininvite_requestsapproved02','admininvite_requestsapproved03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',

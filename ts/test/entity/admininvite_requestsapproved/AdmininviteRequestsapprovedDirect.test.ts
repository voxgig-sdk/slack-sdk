...
describe('AdmininviteRequestsapprovedDirect', async () => {
  ...
  test('direct-load-admininvite_requestsapproved', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-admininvite_requestsapproved', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SLACK_TEST_ADMININVITE_REQUESTSAPPROVED_ENTID']

...
describe('AdminconversationsrestrictAccessDirect', async () => {
  ...
  test('direct-load-adminconversationsrestrict_access', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-adminconversationsrestrict_access', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SLACK_TEST_ADMINCONVERSATIONSRESTRICT_ACCESS_ENTID']

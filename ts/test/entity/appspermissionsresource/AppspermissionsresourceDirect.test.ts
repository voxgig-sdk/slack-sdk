...
describe('AppspermissionsresourceDirect', async () => {
  ...
  test('direct-list-appspermissionsresource', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-appspermissionsresource', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SLACK_TEST_APPSPERMISSIONSRESOURCE_ENTID']

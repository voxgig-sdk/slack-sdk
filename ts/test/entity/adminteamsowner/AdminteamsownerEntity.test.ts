...
describe('AdminteamsownerEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'adminteamsowner.' + op, live)) return
    }
    ...
    let adminteamsowner_ref01_data = Object.values(setup.data.existing.adminteamsowner)[0] as any

    // LOAD
    const adminteamsowner_ref01_ent = client.Adminteamsowner()
    const adminteamsowner_ref01_match_dt0: any = {}
    const adminteamsowner_ref01_data_dt0 = (await adminteamsowner_ref01_ent.load(adminteamsowner_ref01_match_dt0)).data()
    assert(null != adminteamsowner_ref01_data_dt0)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/adminteamsowner/AdminteamsownerTestData.json')
  ...
  let idmap = transform(
    ['adminteamsowner01','adminteamsowner02','adminteamsowner03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',

# Slack JavaScript SDK Reference

Complete API reference for the Slack JavaScript SDK.
...
```ts
const results = await client.Appspermissionsresource().list()
```
...
```ts
const results = await client.Reaction().list()
```
...
| `external_org_migrations` | `Object` | Yes |  |
...
```ts
const results = await client.Usergroupsuser().list()
```

...
class SlackMakeError
{
    public static function call(?SlackContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new SlackContext([], null);
        }
        $op = $ctx->op ?? new SlackOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new SlackResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof SlackError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new SlackError('', $msg, $ctx);

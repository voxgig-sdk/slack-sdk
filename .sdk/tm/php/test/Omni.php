...
//    only - a typed SlackContext there would make every ctx
...
final class SlackOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new SlackContext($clean, null);
    ...
    public static function err_from_map($m): ?SlackError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new SlackError(is_string($code) ? $code : '', $msg);
    }

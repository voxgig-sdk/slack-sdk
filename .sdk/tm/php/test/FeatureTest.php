...
class FtCtrl extends SlackControl
{
...
class FtClient
{
    ...
    private SlackUtility $utility;

    public function __construct(SlackUtility $utility, array $options)
    {
    ...
    public function get_utility(): SlackUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SlackContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (SlackContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SlackConfig::make_feature($name);
    ...
    private static function buildUrl(SlackSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SlackError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SlackResponse($fetched);
        }
    ...
    private function fail(SlackContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SlackContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

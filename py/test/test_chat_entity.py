...
class TestChatEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Chat(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _chat_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "chat." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SLACK_TEST_CHAT_ENTID JSON to run live")
        client = setup["client"]
        ...
        chat_ref01_data = helpers.to_map(runner.entity_data(chat_ref01_ent.create(chat_ref01_data, None)))
        assert chat_ref01_data is not None
...
def _chat_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/chat/ChatTestData.json")

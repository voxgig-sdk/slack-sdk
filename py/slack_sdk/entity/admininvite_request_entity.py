...
class AdmininviteRequestEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AdmininviteRequestEntity(self._client, opts)
    ...
    def data_get(self) -> AdmininviteRequest:
    ...
    def match_get(self) -> AdmininviteRequest:
    ...
    def load(self, reqmatch=None, ctrl=None) -> AdmininviteRequest:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.AdmininviteRequest().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: AdmininviteRequestCreateData, ctrl=None) -> AdmininviteRequest:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,

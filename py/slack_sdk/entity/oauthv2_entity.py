...
class Oauthv2Entity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return Oauthv2Entity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Oauthv2:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Oauthv2().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}

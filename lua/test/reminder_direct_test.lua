...
describe("ReminderDirect", function()
  it("should direct-list-reminder", function()
    local setup = reminder_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-reminder", setup.live and "live" or "unit")
  ...
  it("should direct-load-reminder", function()
    local setup = reminder_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-reminder", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function reminder_direct_setup(mockres)

...
describe("MigrationEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set SLACK_TEST_MIGRATION_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local migration_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.migration")))
    local migration_ref01_data = nil
    if #migration_ref01_data_raw > 0 then
      migration_ref01_data = helpers.to_map(migration_ref01_data_raw[1][2])
    end
...
function migration_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("SLACK_TEST_MIGRATION_ENTID")

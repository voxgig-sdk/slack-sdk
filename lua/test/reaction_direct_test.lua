...
describe("ReactionDirect", function()
  it("should direct-list-reaction", function()
    local setup = reaction_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-reaction", setup.live and "live" or "unit")
  ...
  it("should direct-load-reaction", function()
    local setup = reaction_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-reaction", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function reaction_direct_setup(mockres)

...
It exposes the API as capitalised, semantic **Entities** — e.g. `client:Conversationsinfo()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
Create an instance: `local conversationsinfo = client:Conversationsinfo(nil)`
...
Create an instance: `local conversationslist = client:Conversationslist(nil)`

...
ReminderEntity.__index = ReminderEntity


function ReminderEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReminderEntity)
  self._name = "reminder"
  self._client = client
...
function ReminderEntity:get_name()
  return self._name
end


function ReminderEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReminderEntity.new(self._client, opts)
end
...
function ReminderEntity:mark_deleted()
  self._deleted = true
end


function ReminderEntity:deleted()
  return true == self._deleted
end


function ReminderEntity:data_set(args)
  if args ~= nil then
...
function ReminderEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReminderEntity:match_set(args)
  if args ~= nil then
...
function ReminderEntity:match_get()
...
function ReminderEntity:stream(action, args, callopts)
...
---@param reqmatch ReminderLoadMatch
---@param ctrl? table
---@return Reminder
---@return string? err
function ReminderEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ReminderListMatch
---@param ctrl? table
---@return Reminder[]
---@return string? err
function ReminderEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata ReminderCreateData
---@param ctrl? table
---@return Reminder
---@return string? err
function ReminderEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function ReminderEntity:_run_op(ctx, post_done)
  local utility = self._utility

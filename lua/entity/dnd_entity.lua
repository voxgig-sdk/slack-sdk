...
local DndEntity = {}
DndEntity.__index = DndEntity


function DndEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DndEntity)
  self._name = "dnd"
  self._client = client
...
function DndEntity:get_name()
  return self._name
end


function DndEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DndEntity.new(self._client, opts)
end
...
function DndEntity:mark_deleted()
  self._deleted = true
end


function DndEntity:deleted()
  return true == self._deleted
end


function DndEntity:data_set(args)
  if args ~= nil then
...
function DndEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DndEntity:match_set(args)
  if args ~= nil then
...
function DndEntity:stream(action, args, callopts)
...
---@param reqmatch DndLoadMatch
---@param ctrl? table
---@return Dnd
---@return string? err
function DndEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata DndCreateData
---@param ctrl? table
---@return Dnd
---@return string? err
function DndEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function DndEntity:_run_op(ctx, post_done)
  local utility = self._utility

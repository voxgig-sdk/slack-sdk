...
local PinEntity = {}
PinEntity.__index = PinEntity


function PinEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PinEntity)
  self._name = "pin"
  self._client = client
...
function PinEntity:get_name()
  return self._name
end


function PinEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PinEntity.new(self._client, opts)
end
...
function PinEntity:mark_deleted()
  self._deleted = true
end


function PinEntity:deleted()
  return true == self._deleted
end


function PinEntity:data_set(args)
  if args ~= nil then
...
function PinEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PinEntity:match_set(args)
  if args ~= nil then
...
function PinEntity:stream(action, args, callopts)
...
---@param reqmatch PinLoadMatch
---@param ctrl? table
---@return Pin
---@return string? err
function PinEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata PinCreateData
---@param ctrl? table
---@return Pin
---@return string? err
function PinEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function PinEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
local ViewEntity = {}
ViewEntity.__index = ViewEntity


function ViewEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ViewEntity)
  self._name = "view"
  self._client = client
...
function ViewEntity:get_name()
  return self._name
end


function ViewEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ViewEntity.new(self._client, opts)
end
...
function ViewEntity:mark_deleted()
  self._deleted = true
end


function ViewEntity:deleted()
  return true == self._deleted
end


function ViewEntity:data_set(args)
  if args ~= nil then
...
function ViewEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ViewEntity:match_set(args)
  if args ~= nil then
...
function ViewEntity:stream(action, args, callopts)
...
---@param reqmatch ViewLoadMatch
---@param ctrl? table
---@return View
---@return string? err
function ViewEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ViewEntity:_run_op(ctx, post_done)
  local utility = self._utility

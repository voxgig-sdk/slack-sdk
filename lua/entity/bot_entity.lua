...
local BotEntity = {}
BotEntity.__index = BotEntity


function BotEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BotEntity)
  self._name = "bot"
  self._client = client
...
function BotEntity:get_name()
  return self._name
end


function BotEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BotEntity.new(self._client, opts)
end
...
function BotEntity:mark_deleted()
  self._deleted = true
end


function BotEntity:deleted()
  return true == self._deleted
end


function BotEntity:data_set(args)
  if args ~= nil then
...
function BotEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BotEntity:match_set(args)
  if args ~= nil then
...
function BotEntity:stream(action, args, callopts)
...
---@param reqmatch BotLoadMatch
---@param ctrl? table
---@return Bot
---@return string? err
function BotEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function BotEntity:_run_op(ctx, post_done)
  local utility = self._utility

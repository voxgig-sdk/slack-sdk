...
local AuthEntity = {}
AuthEntity.__index = AuthEntity


function AuthEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AuthEntity)
  self._name = "auth"
  self._client = client
...
function AuthEntity:get_name()
  return self._name
end


function AuthEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AuthEntity.new(self._client, opts)
end
...
function AuthEntity:mark_deleted()
  self._deleted = true
end


function AuthEntity:deleted()
  return true == self._deleted
end


function AuthEntity:data_set(args)
  if args ~= nil then
...
function AuthEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AuthEntity:match_set(args)
  if args ~= nil then
...
function AuthEntity:stream(action, args, callopts)
...
---@param reqmatch AuthLoadMatch
---@param ctrl? table
---@return Auth
---@return string? err
function AuthEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function AuthEntity:_run_op(ctx, post_done)
  local utility = self._utility

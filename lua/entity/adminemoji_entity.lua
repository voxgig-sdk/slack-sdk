...
local AdminemojiEntity = {}
AdminemojiEntity.__index = AdminemojiEntity


function AdminemojiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AdminemojiEntity)
  self._name = "adminemoji"
  self._client = client
...
function AdminemojiEntity:get_name()
  return self._name
end


function AdminemojiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AdminemojiEntity.new(self._client, opts)
end
...
function AdminemojiEntity:mark_deleted()
  self._deleted = true
end


function AdminemojiEntity:deleted()
  return true == self._deleted
end


function AdminemojiEntity:data_set(args)
  if args ~= nil then
...
function AdminemojiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AdminemojiEntity:match_set(args)
  if args ~= nil then
...
function AdminemojiEntity:match_get()
...
function AdminemojiEntity:stream(action, args, callopts)
...
---@param reqmatch AdminemojiLoadMatch
---@param ctrl? table
---@return Adminemoji
---@return string? err
function AdminemojiEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata AdminemojiCreateData
---@param ctrl? table
---@return Adminemoji
---@return string? err
function AdminemojiEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function AdminemojiEntity:_run_op(ctx, post_done)
  local utility = self._utility

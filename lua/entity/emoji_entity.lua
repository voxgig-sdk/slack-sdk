...
local EmojiEntity = {}
EmojiEntity.__index = EmojiEntity


function EmojiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EmojiEntity)
  self._name = "emoji"
  self._client = client
...
function EmojiEntity:get_name()
  return self._name
end


function EmojiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EmojiEntity.new(self._client, opts)
end
...
function EmojiEntity:mark_deleted()
  self._deleted = true
end


function EmojiEntity:deleted()
  return true == self._deleted
end


function EmojiEntity:data_set(args)
  if args ~= nil then
...
function EmojiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EmojiEntity:match_set(args)
  if args ~= nil then
...
function EmojiEntity:match_get()
...
function EmojiEntity:stream(action, args, callopts)
...
---@param reqmatch EmojiLoadMatch
---@param ctrl? table
---@return Emoji
---@return string? err
function EmojiEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function EmojiEntity:_run_op(ctx, post_done)
  local utility = self._utility

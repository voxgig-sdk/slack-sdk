...
local DialogEntity = {}
DialogEntity.__index = DialogEntity


function DialogEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DialogEntity)
  self._name = "dialog"
  self._client = client
...
function DialogEntity:get_name()
  return self._name
end


function DialogEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DialogEntity.new(self._client, opts)
end
...
function DialogEntity:mark_deleted()
  self._deleted = true
end


function DialogEntity:deleted()
  return true == self._deleted
end


function DialogEntity:data_set(args)
  if args ~= nil then
...
function DialogEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DialogEntity:match_set(args)
  if args ~= nil then
...
function DialogEntity:match_get()
...
function DialogEntity:stream(action, args, callopts)
...
---@param reqmatch DialogLoadMatch
---@param ctrl? table
---@return Dialog
---@return string? err
function DialogEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DialogEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
local AdminappEntity = {}
AdminappEntity.__index = AdminappEntity


function AdminappEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AdminappEntity)
  self._name = "adminapp"
  self._client = client
...
function AdminappEntity:get_name()
  return self._name
end


function AdminappEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AdminappEntity.new(self._client, opts)
end
...
function AdminappEntity:mark_deleted()
  self._deleted = true
end


function AdminappEntity:deleted()
  return true == self._deleted
end


function AdminappEntity:data_set(args)
  if args ~= nil then
...
function AdminappEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AdminappEntity:match_set(args)
  if args ~= nil then
...
function AdminappEntity:match_get()
...
function AdminappEntity:stream(action, args, callopts)
...
---@param reqdata AdminappCreateData
---@param ctrl? table
---@return Adminapp
---@return string? err
function AdminappEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function AdminappEntity:_run_op(ctx, post_done)
  local utility = self._utility

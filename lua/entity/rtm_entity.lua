...
local RtmEntity = {}
RtmEntity.__index = RtmEntity


function RtmEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RtmEntity)
  self._name = "rtm"
  self._client = client
...
function RtmEntity:get_name()
  return self._name
end


function RtmEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RtmEntity.new(self._client, opts)
end
...
function RtmEntity:mark_deleted()
  self._deleted = true
end


function RtmEntity:deleted()
  return true == self._deleted
end


function RtmEntity:data_set(args)
  if args ~= nil then
...
function RtmEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RtmEntity:match_set(args)
  if args ~= nil then
...
function RtmEntity:stream(action, args, callopts)
...
---@param reqmatch RtmLoadMatch
---@param ctrl? table
---@return Rtm
---@return string? err
function RtmEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function RtmEntity:_run_op(ctx, post_done)
  local utility = self._utility

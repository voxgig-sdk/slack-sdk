...
local StarEntity = {}
StarEntity.__index = StarEntity


function StarEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StarEntity)
  self._name = "star"
  self._client = client
...
function StarEntity:get_name()
  return self._name
end


function StarEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StarEntity.new(self._client, opts)
end
...
function StarEntity:mark_deleted()
  self._deleted = true
end


function StarEntity:deleted()
  return true == self._deleted
end


function StarEntity:data_set(args)
  if args ~= nil then
...
function StarEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StarEntity:match_set(args)
  if args ~= nil then
...
function StarEntity:stream(action, args, callopts)
...
---@param reqmatch StarListMatch
---@param ctrl? table
---@return Star[]
---@return string? err
function StarEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata StarCreateData
---@param ctrl? table
---@return Star
---@return string? err
function StarEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function StarEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
local ReactionEntity = {}
ReactionEntity.__index = ReactionEntity


function ReactionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReactionEntity)
  self._name = "reaction"
  self._client = client
...
function ReactionEntity:get_name()
  return self._name
end


function ReactionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReactionEntity.new(self._client, opts)
end
...
function ReactionEntity:mark_deleted()
  self._deleted = true
end


function ReactionEntity:deleted()
  return true == self._deleted
end


function ReactionEntity:data_set(args)
  if args ~= nil then
...
function ReactionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReactionEntity:match_set(args)
  if args ~= nil then
...
function ReactionEntity:match_get()
...
function ReactionEntity:stream(action, args, callopts)
...
---@param reqmatch ReactionLoadMatch
---@param ctrl? table
---@return Reaction
---@return string? err
function ReactionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ReactionListMatch
---@param ctrl? table
---@return Reaction[]
---@return string? err
function ReactionEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata ReactionCreateData
---@param ctrl? table
---@return Reaction
---@return string? err
function ReactionEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function ReactionEntity:_run_op(ctx, post_done)
  local utility = self._utility

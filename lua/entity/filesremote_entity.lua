...
local FilesremoteEntity = {}
FilesremoteEntity.__index = FilesremoteEntity


function FilesremoteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FilesremoteEntity)
  self._name = "filesremote"
  self._client = client
...
function FilesremoteEntity:get_name()
  return self._name
end


function FilesremoteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FilesremoteEntity.new(self._client, opts)
end
...
function FilesremoteEntity:mark_deleted()
  self._deleted = true
end


function FilesremoteEntity:deleted()
  return true == self._deleted
end


function FilesremoteEntity:data_set(args)
  if args ~= nil then
...
function FilesremoteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FilesremoteEntity:match_set(args)
  if args ~= nil then
...
function FilesremoteEntity:match_get()
...
function FilesremoteEntity:stream(action, args, callopts)
...
---@param reqmatch FilesremoteLoadMatch
---@param ctrl? table
---@return Filesremote
---@return string? err
function FilesremoteEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata FilesremoteCreateData
---@param ctrl? table
---@return Filesremote
---@return string? err
function FilesremoteEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function FilesremoteEntity:_run_op(ctx, post_done)
  local utility = self._utility

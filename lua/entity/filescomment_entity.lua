...
local FilescommentEntity = {}
FilescommentEntity.__index = FilescommentEntity


function FilescommentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FilescommentEntity)
  self._name = "filescomment"
  self._client = client
...
function FilescommentEntity:get_name()
  return self._name
end


function FilescommentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FilescommentEntity.new(self._client, opts)
end
...
function FilescommentEntity:mark_deleted()
  self._deleted = true
end


function FilescommentEntity:deleted()
  return true == self._deleted
end


function FilescommentEntity:data_set(args)
  if args ~= nil then
...
function FilescommentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FilescommentEntity:match_set(args)
  if args ~= nil then
...
function FilescommentEntity:match_get()
...
function FilescommentEntity:stream(action, args, callopts)
...
---@param reqdata FilescommentCreateData
---@param ctrl? table
---@return Filescomment
---@return string? err
function FilescommentEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function FilescommentEntity:_run_op(ctx, post_done)
  local utility = self._utility

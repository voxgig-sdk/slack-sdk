...
local AdminuserEntity = {}
AdminuserEntity.__index = AdminuserEntity


function AdminuserEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AdminuserEntity)
  self._name = "adminuser"
  self._client = client
...
function AdminuserEntity:get_name()
  return self._name
end


function AdminuserEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AdminuserEntity.new(self._client, opts)
end
...
function AdminuserEntity:mark_deleted()
  self._deleted = true
end


function AdminuserEntity:deleted()
  return true == self._deleted
end


function AdminuserEntity:data_set(args)
  if args ~= nil then
...
function AdminuserEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AdminuserEntity:match_set(args)
  if args ~= nil then
...
function AdminuserEntity:match_get()
...
function AdminuserEntity:stream(action, args, callopts)
...
---@param reqmatch AdminuserLoadMatch
---@param ctrl? table
---@return Adminuser
---@return string? err
function AdminuserEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata AdminuserCreateData
---@param ctrl? table
---@return Adminuser
---@return string? err
function AdminuserEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function AdminuserEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
local ChatEntity = {}
ChatEntity.__index = ChatEntity


function ChatEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ChatEntity)
  self._name = "chat"
  self._client = client
...
function ChatEntity:get_name()
  return self._name
end


function ChatEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ChatEntity.new(self._client, opts)
end
...
function ChatEntity:mark_deleted()
  self._deleted = true
end


function ChatEntity:deleted()
  return true == self._deleted
end


function ChatEntity:data_set(args)
  if args ~= nil then
...
function ChatEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ChatEntity:match_set(args)
  if args ~= nil then
...
function ChatEntity:stream(action, args, callopts)
...
---@param reqdata ChatCreateData
---@param ctrl? table
---@return Chat
---@return string? err
function ChatEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function ChatEntity:_run_op(ctx, post_done)
  local utility = self._utility

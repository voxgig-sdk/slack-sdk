...
boru-driven command-line client **and** interactive REPL for the Slack
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/slack-cli)
make build
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/slack-cli
   ```
...
Run with no arguments to open a REPL (prompt `slack>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/slack-cli`. |
...
(`./slack-cli <expr>`) and interactively (the REPL), and expressions compose the

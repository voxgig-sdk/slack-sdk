...
The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Conversationsinfo()` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
Create an instance: `$conversationsinfo = $client->Conversationsinfo();`
...
Create an instance: `$conversationslist = $client->Conversationslist();`

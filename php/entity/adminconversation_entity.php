...
// Slack SDK Adminconversation entity
...
class AdminconversationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new AdminconversationEntity($this->_client, $opts);
    }

    /**
     * @param Adminconversation|array $args Adminconversation data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = SlackHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Adminconversation|array The current Adminconversation data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Adminconversation fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = SlackHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Adminconversation fields).
     */
    ...
    /**
     * Load a single Adminconversation.
     *
     * @param AdminconversationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed AdminconversationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Adminconversation|array The loaded Adminconversation as an assoc-array at the
     *   SDK boundary; throws SlackError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = SlackHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Adminconversation items matching the given filter.
     *
     * @param AdminconversationListMatch|array|null $reqmatch Match filter (any subset
     *   of Adminconversation fields) as an assoc-array; AdminconversationListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Adminconversation[]|array A list of Adminconversation items as assoc-arrays at
     *   the SDK boundary; throws SlackError on failure (item-5 convention).
     */
    ...
    /**
     * Create a new Adminconversation.
     *
     * @param AdminconversationCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed AdminconversationCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Adminconversation|array The created Adminconversation as an assoc-array at the
     *   SDK boundary; throws SlackError on failure (item-5 convention).
     */

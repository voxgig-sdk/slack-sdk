...
class SlackSDK
{
    ...
    private $_conversationsinfo = null;

    // Canonical facade: $client->Conversationsinfo()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->conversationsinfo()
    // resolves here too.
    public function Conversationsinfo($data = null)
    {
        require_once __DIR__ . '/entity/conversationsinfo_entity.php';
        if ($data === null) {
            if ($this->_conversationsinfo === null) {
                $this->_conversationsinfo = new ConversationsinfoEntity($this, null);
            }
            return $this->_conversationsinfo;
        }
        return new ConversationsinfoEntity($this, $data);
    }


    private $_conversationslist = null;

    // Canonical facade: $client->Conversationslist()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->conversationslist()
    // resolves here too.
    public function Conversationslist($data = null)
    {
        require_once __DIR__ . '/entity/conversationslist_entity.php';
        if ($data === null) {
            if ($this->_conversationslist === null) {
                $this->_conversationslist = new ConversationslistEntity($this, null);
            }
            return $this->_conversationslist;
        }
        return new ConversationslistEntity($this, $data);
    }

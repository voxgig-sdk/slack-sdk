...
class ReactionEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SLACK_TEST_REACTION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function reaction_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["reaction01", "reaction02", "reaction03"] as $k) {
    ...
    $entid_env_raw = getenv("SLACK_TEST_REACTION_ENTID");

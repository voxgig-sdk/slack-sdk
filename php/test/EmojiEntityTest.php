...
class EmojiEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SLACK_TEST_EMOJI_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $emoji_ref01_data = null;
        if (count($emoji_ref01_data_raw) > 0) {
        ...
        $this->assertNotNull($emoji_ref01_data_dt0_loaded);
...
function emoji_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["emoji01", "emoji02", "emoji03"] as $k) {
    ...
    $entid_env_raw = getenv("SLACK_TEST_EMOJI_ENTID");

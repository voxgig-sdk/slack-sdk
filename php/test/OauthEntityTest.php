...
class OauthEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SLACK_TEST_OAUTH_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $oauth_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.oauth")));
        $oauth_ref01_data = null;
        if (count($oauth_ref01_data_raw) > 0) {
            $oauth_ref01_data = Helpers::to_map($oauth_ref01_data_raw[0][1]);
        }
...
function oauth_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("SLACK_TEST_OAUTH_ENTID");

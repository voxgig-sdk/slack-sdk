...
{
    private static function utility(): SlackUtility
    {
        return new SlackUtility();
    }
    ...
    private static function ctx(array $over = []): SlackContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new SlackContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new SlackOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof SlackError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new SlackOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = SlackMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new SlackOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = SlackMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new SlackOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = SlackMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new SlackSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = SlackMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        ...
        [, $err] = SlackMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new SlackSpec(['step' => 's']),
            'response' => new SlackResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new SlackResult([]),
        ]);
        [, $err] = SlackMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new SlackSpec(['step' => 's']),
            'response' => new SlackResponse(self::resp(200, ['v' => 1])),
            'result' => new SlackResult([]),
        ]);
        [, $err] = SlackMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new SlackSpec(['step' => 's']),
            'response' => new SlackResponse(self::resp(200, ['v' => 2])),
            'result' => new SlackResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        SlackMakeResponse::call($ctx);
    ...
    {
        $preset = new SlackResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new SlackSpec([]),
            'response' => new SlackResponse([]),
            'result' => new SlackResult([]),
        ...
        [$got, $err] = SlackMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new SlackResult([])]);
        [, $err] = SlackMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new SlackSpec([]), 'result' => null]);
        [, $err] = SlackMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new SlackSpec(['step' => 's']),
            'result' => new SlackResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new SlackOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = SlackMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new SlackSpec(['step' => 's']),
            'result' => new SlackResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new SlackOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = SlackMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new SlackResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new SlackSpec([]),
            'result' => new SlackResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = SlackMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new SlackError('boom', 'boom');
        $utility->fetcher = function (SlackContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new SlackSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = SlackMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (SlackContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = SlackMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(SlackResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (SlackContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new SlackSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = SlackMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new SlackResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new SlackSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = SlackMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = SlackMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = SlackMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new SlackResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, SlackDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new SlackResult(['ok' => false])]);
        $this->expectException(SlackError::class);
        SlackDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new SlackResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', SlackMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new SlackBaseFeature();
        $b = new SlackBaseFeature();
        SlackFeatureAdd::call($ctx, $a);
        SlackFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): SlackBaseFeature
    {
        $f = new SlackBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): SlackContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new SlackSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new SlackResponse(['headers' => null]),
            'result' => new SlackResult([]),
        ]);
        SlackResultHeaders::call($ctx);

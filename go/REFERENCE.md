...
#### `Conversationsinfo(data map[string]any) SlackEntity`

Create a new `Conversationsinfo` entity instance. Pass `nil` for no initial data.

#### `Conversationslist(data map[string]any) SlackEntity`
...
fmt.Println(conversationsinfo.GetName()) // "conversationsinfo"
```
...
```go
result, err := client.Conversationsinfo(nil).Load(map[string]any{"id": "conversationsinfo_id"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
...
```go
conversationslist := client.Conversationslist(nil)
fmt.Println(conversationslist.GetName()) // "conversationslist"
```
...
```go
results, err := client.Conversationslist(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(results)

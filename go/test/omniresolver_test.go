...
import (
	...
	omni "github.com/voxgig-sdk/slack-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SlackError); is {
				out := map[string]any{
					"name":    "SlackError",
...
func omniCtx(arg any, client *sdk.SlackSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SlackSDK); is {
			client = live

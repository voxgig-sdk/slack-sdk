...
func TestReactionEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Reaction(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Reaction(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SLACK_TEST_REACTION_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		reactionRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "reaction"}), "reaction_ref01"))

		reactionRef01DataResult, err := reactionRef01Ent.Create(reactionRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		reactionRef01Data = core.ToMapAny(entityData(reactionRef01DataResult))
		if reactionRef01Data == nil {
			t.Fatal("expected create result to be a map")
		}

		// LIST
		reactionRef01Match := map[string]any{}

		reactionRef01ListResult, err := reactionRef01Ent.List(reactionRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, reactionRef01ListOk := reactionRef01ListResult.([]any)
		if !reactionRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", reactionRef01ListResult)
		}
...
func reactionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"reaction01", "reaction02", "reaction03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("SLACK_TEST_REACTION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SLACK_TEST_REACTION_ENTID"])

...
func TestEmojiEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SLACK_TEST_EMOJI_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		emojiRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.emoji")))
		var emojiRef01Data map[string]any
		if len(emojiRef01DataRaw) > 0 {
			emojiRef01Data = core.ToMapAny(emojiRef01DataRaw[0][1])
		}
		...
		if emojiRef01DataDt0Loaded == nil {
...
func emojiBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SLACK_TEST_EMOJI_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SLACK_TEST_EMOJI_ENTID"])

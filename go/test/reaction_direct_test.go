...
func TestReactionDirect(t *testing.T) {
	...
	t.Run("direct-load-reaction", func(t *testing.T) {
		setup := reactionDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-reaction", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func reactionDirectSetup(mockres any) *reactionDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"SLACK_TEST_REACTION_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["SLACK_TEST_REACTION_ENTID"]; ok {

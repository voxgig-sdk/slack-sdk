...
func TestApiEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SLACK_TEST_API_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func apiBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SLACK_TEST_API_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SLACK_TEST_API_ENTID"])

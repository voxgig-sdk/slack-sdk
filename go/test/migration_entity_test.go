...
func TestMigrationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SLACK_TEST_MIGRATION_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		migrationRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.migration")))
		var migrationRef01Data map[string]any
		if len(migrationRef01DataRaw) > 0 {
			migrationRef01Data = core.ToMapAny(migrationRef01DataRaw[0][1])
		}
		// Discard guards against Go's unused-var check when the flow's steps
		// happen not to consume the bootstrap data (e.g. list-only flows).
		_ = migrationRef01Data
		...
		_, migrationRef01ListOk := migrationRef01ListResult.([]any)
...
func migrationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"migration01", "migration02", "migration03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("SLACK_TEST_MIGRATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SLACK_TEST_MIGRATION_ENTID"])
